import { getFileFromRelativePath, getHashSuffix, getPathWithoutSuffix } from "../../api/localFiles";
...
export function createNavigationActions(context: ReaderNavigationContext) {
  ...
  async function goUp(): Promise<void> {
    ...
    const handle = await getDirectoryByStack(context.stack.value.slice(1, -1));
  ...
  async function openRelativeDocument(rawHref: string): Promise<string> {
    ...
    const result = await getFileFromRelativePath(context.rootHandle.value, context.currentFileDirectoryPath.value, cleanPath);
    if (!result) return "";
   ...
   * @returns 目录句柄。
   */
  async function getDirectoryByStack(names: string[]): Promise<FileSystemDirectoryHandleLike> {
    let handle = context.rootHandle.value as FileSystemDirectoryHandleLike;
    for (const name of names) handle = await handle.getDirectoryHandle(name);
    return handle;
  }
